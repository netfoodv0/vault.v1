...
export function useCardapio() {
  ...
  const deleteImage = async (imageUrl: string) => {
    if (!imageUrl || imageUrl === '/product-placeholder.jpg') return
    ...
    try {
      ...
      // Não lance erro aqui para não interromper outras operações
    }
  }
  ...
  const deleteProduct = async (productId: string) => {
    ...
    try {
      ...
      if (product?.imageUrl && product.imageUrl !== '/product-placeholder.jpg') {
  ...
  const deleteAdditional = async (additionalId: string) => {
    ...
    try {
      ...
      if (additional?.imageUrl && additional.imageUrl !== '/product-placeholder.jpg') {
        await deleteImage(additional.imageUrl)
      }
